...
public class CountUseExecutor {
    public void executeQuery(String database) throws Exception {
        Map<String,Integer> useQueryCount =new HashMap<String, Integer>();
        ...
        while(logFileReader.hasNextLine()) {
            ...
            if (data.length() != 0) {
                if(database.equals(data.split("\\|")[1].replace(";",""))){
                    ...
                    boolean use = data.contains("use ");
                    if(valid && use){
                        if (useQueryCount.containsKey(user)) {
                            int currentCount = useQueryCount.get(user);
                            int updatedCount = currentCount + 1;
                            useQueryCount.put(user, updatedCount);
                        } else {
                            useQueryCount.put(user, 1);
                        }
                    }
                }
            }
        }
        for(String user: useQueryCount.keySet()){
            System.out.println("user "+user+" submitted "+ useQueryCount.get(user)+" use queries on "+database);
        }
